...
class TurTurSegmentSummary:
    ...
    Each :class:`TurTur<olexparser.turtur.TurTur>` in the :class:`TurDataFile<olexparser.turdata_file.TurDataFile>`
    lists 1 or more segment files associated to it.

    A summary of the segment file is also stored in the :class:`TurDataFile<olexparser.turdata_file.TurDataFile>`.
    ...
    :var self.seg_num: A number identifying the related Segment filename. i.e. if the segment number is 83 the
                        filename will be "segment83_A".
    :var self.num_entries: he number of entries in the segment file. Each entry is 16 bytes in length. This number
                        can be used to determine the expected file size of the segment.
    :var self.smallest_lat: An "Olex float" containing the smallest Latitude position in the segment file.
    :var self.smallest_long: An "Olex float" containing the smallest Latitude position in the segment file.
    :var self.largest_lat: An "Olex float" containing the largest Latitude position in the segment file.
    :var self.largest_long: An "Olex float" containing the largest Longitude position in the segment file.
    :var self.smallest_time: A Unix Timestamp of the first entry in the segment file.
    :var self.largest_time: A Unix Timestamp of the last entry in the segment file.
    :var self.warnings: A list of warnings generated by the
        :class:`TurTurSegmentSummary<olexparser.turtur_segment_summary.TurTurSegmentSummary>`
    ...
    def get_entries_num(self):
        """
        :return: the number of entries in the :class:`SegmentFile<olexparser.segment_file.SegmentFile>`
        :rtype: int
        """
